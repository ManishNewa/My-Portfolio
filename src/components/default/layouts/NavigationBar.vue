        ...
        <div class="max-w-6xl mx-auto flex justify-center items-center">
            <div class="hidden md:flex space-x-8">
                <a v-for="item in navItems" :key="item.id" :href="item.href"
                    class="text-gray-700 hover:text-purple-600 transition-colors duration-300 relative overflow-hidden group p-2">
                    <span class="relative z-10">{{ item.label }}</span>
                    <!-- Hover Underline -->
                    <span
                        class="absolute bottom-0 left-0 w-full h-0.5 bg-purple-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
                </a>
            </div>
...
<style scoped>
/* Custom hover animation for desktop navigation */
.group:hover .scale-0 {
    transform: scale(1);
}
